...
- **Clerk new-device verification:** Clerk enforces email code verification when signing in from a new device/VM. There are no alternative methods configured (no TOTP/SMS). This blocks automated sign-in from cloud VMs. The user must log in via the Desktop pane or disable new-device verification in the Clerk dashboard for the dev instance.
